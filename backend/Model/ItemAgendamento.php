...
// ---------------- MÉTODOS DE BUSCA ----------------
    ...
    public function totalDeItensAgendamento() {
    $sql = "SELECT COUNT(*) as total FROM tbl_item_agendamento";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_COLUMN);
}

public function totalDeItensAgendamentoAtivos() {
    $sql = "SELECT COUNT(*) as total FROM tbl_item_agendamento WHERE excluido_em IS NULL";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_COLUMN);
}

public function totalDeItensAgendamentoInativos() {
    $sql = "SELECT COUNT(*) as total FROM tbl_item_agendamento WHERE excluido_em IS NOT NULL";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_COLUMN);
}
public function paginacao(int $pagina = 1, int $por_pagina = 10): array{
        $totalQuery = "SELECT COUNT(*) FROM `tbl_item_agendamento`";
        $totalStmt = $this->db->query($totalQuery);
        $total_de_registros = $totalStmt->fetchColumn();
        $offset = ($pagina - 1) * $por_pagina;
        $dataQuery = "SELECT * FROM `tbl_item_agendamento` LIMIT :limit OFFSET :offset";
