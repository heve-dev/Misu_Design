...
class Contato{
 ...
 // contrutor inicializa a classe e ou atributos
  ...
  // ---------------- MÉTODOS DE BUSCA ----------------
    ...
    public function totalDeContatos() {
...
public function totalDeContatosAtivos() {
    $sql = "SELECT COUNT(*) as total FROM tbl_contato WHERE excluido_em IS NULL";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_COLUMN);
}

public function totalDeContatosInativos() {
    $sql = "SELECT COUNT(*) as total FROM tbl_contato WHERE excluido_em IS NOT NULL";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_COLUMN);
}
public function paginacao(int $pagina = 1, int $por_pagina = 10): array{
        $totalQuery = "SELECT COUNT(*) FROM `tbl_contato`";
        $totalStmt = $this->db->query($totalQuery);
        $total_de_registros = $totalStmt->fetchColumn();
        $offset = ($pagina - 1) * $por_pagina;
        $dataQuery = "SELECT * FROM `tbl_contato` LIMIT :limit OFFSET :offset";
